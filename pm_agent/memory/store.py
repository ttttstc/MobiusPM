...
class Store:
    ...
    def count_by_owner_today(self, welink_id: str, today: str | None = None) -> int:
        ...
        cur = self._conn.execute(
            ...
            "WHERE welink_id=? AND created_at LIKE ? AND send_status IN ('success','mock')",
            (welink_id, f"{d}%"),
        )
    ...
    def count_run(self, run_id: str) -> int:
        cur = self._conn.execute(
            "SELECT COUNT(*) FROM follow_up_log "
            "WHERE run_id=? AND send_status IN ('success','mock')",
            (run_id,),
        )
        return cur.fetchone()[0]
    ...
    def exists_dedupe(self, dedupe_key: str) -> bool:
        cur = self._conn.execute(
            "SELECT 1 FROM follow_up_log WHERE dedupe_key=? AND send_status IN ('success','mock') LIMIT 1",
            (dedupe_key,),
        )
